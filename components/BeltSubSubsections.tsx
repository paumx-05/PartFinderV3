// components/BeltSubSubsections.tsx
'use client';
...
import BeltFilters from './BeltFilters';
import { beltSubSubsections } from '@/lib/mocks/beltSubSubsections';

interface BeltSubSubsectionsProps {
  vehicleData: {
    vin: string;
    brand: string;
    model: string;
...
export default function BeltSubSubsections({ 
  ...
  const currentSubSubcategory = beltSubSubsections.find(sub => sub.id === activeSubSubcategory);
