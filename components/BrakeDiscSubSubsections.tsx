// components/BrakeDiscSubSubsections.tsx
'use client';
...
import BrakeDiscFilters from './BrakeDiscFilters';
import { brakeDiscSubSubsections } from '@/lib/mocks/brakeDiscSubSubsections';

interface BrakeDiscSubSubsectionsProps {
  vehicleData: {
    vin: string;
    brand: string;
    model: string;
...
export default function BrakeDiscSubSubsections({ 
  vehicleData, 
  onAddToCart, 
  cartItemsCount = 0,
  onBackToSubcategory
}: BrakeDiscSubSubsectionsProps) {
  const [activeSubSubcategory, setActiveSubSubcategory] = useState('brake-discs');
  const [viewMode, setViewMode] = useState<ViewMode>('list');
  const [searchQuery, setSearchQuery] = useState('');
  const [brandFilter, setBrandFilter] = useState('');
  const [materialFilter, setMaterialFilter] = useState('');

  const currentSubSubcategory = brakeDiscSubSubsections.find(subsub => subsub.id === activeSubSubcategory);
  ...
  return (
    <div className="space-y-6">
      ...
      <SubSubsectionHeader
        ...
        subcategoryName="Freno de disco"
      ...
      <SubSubCategoriesNavigation
        subsubcategories={brakeDiscSubSubsections}
        activeSubSubcategory={activeSubSubcategory}
        onSubSubcategoryChange={handleSubSubcategoryChange}
      />

      {/* Filtros específicos para freno de disco */}
      <BrakeDiscFilters
      ...
      <PartsToolbar
        category={{
          ...
          name: 'Freno de disco',
          icon: 'Disc',
          description: 'Discos de freno, pastillas, pinzas y sensores de desgaste',
          parts: []
        }}
