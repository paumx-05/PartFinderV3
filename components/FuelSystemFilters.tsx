// components/FuelSystemFilters.tsx
'use client';
import React from 'react';
import { Filter, X } from 'lucide-react';

interface FuelSystemFiltersProps {
...
const brandOptions = [
  ...
  { value: 'WIX', label: 'WIX' },
  { value: 'Stant', label: 'Stant' },
  { value: 'Pierburg', label: 'Pierburg' },
...
const materialOptions = [
  ...
  { value: 'goma', label: 'Goma EPDM' },
  { value: 'aluminio', label: 'Aluminio' },
  { value: 'latón', label: 'Latón' },
  { value: 'papel', label: 'Papel filtrante' },
  { value: 'cerámica', label: 'Cerámica' },
  { value: 'cobre', label: 'Cobre' },
  { value: 'níquel', label: 'Níquel' },
];

export default function FuelSystemFilters({
  onBrandFilter,
  onMaterialFilter,
  onClearFilters,
  activeBrand,
  activeMaterial,
}: FuelSystemFiltersProps) {
    ...
    <div className="bg-gray-800 bg-opacity-70 rounded-lg p-4 border-l-4 border-red-600">
      <div className="flex items-center justify-between mb-4">
        <h5 className="text-sm font-bold text-red-600 flex items-center space-x-2">
          ...
          <span>Filtros de Alimentación de Combustible</span>
        </h5>
        {hasActiveFilters && (
          <button
