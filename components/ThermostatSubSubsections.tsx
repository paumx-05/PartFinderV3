// components/ThermostatSubSubsections.tsx
'use client';
...
import ThermostatFilters from './ThermostatFilters';
import { thermostatSubSubsections } from '@/lib/mocks/thermostatSubSubsections';

interface ThermostatSubSubsectionsProps {
  vehicleData: {
    vin: string;
    brand: string;
    model: string;
...
export default function ThermostatSubSubsections({ 
  vehicleData, 
  onAddToCart, 
  cartItemsCount = 0,
  onBackToSubcategory
}: ThermostatSubSubsectionsProps) {
  const [activeSubSubcategory, setActiveSubSubcategory] = useState('thermostat-complete');
  const [viewMode, setViewMode] = useState<ViewMode>('grid');
  const [searchQuery, setSearchQuery] = useState('');
  const [brandFilter, setBrandFilter] = useState('');
  const [materialFilter, setMaterialFilter] = useState('');

  const currentSubSubcategory = thermostatSubSubsections.find(subsub => subsub.id === activeSubSubcategory);
  ...
  return (
    <div className="space-y-6">
      ...
      <SubSubCategoriesNavigation
        subsubcategories={thermostatSubSubsections}
        activeSubSubcategory={activeSubSubcategory}
        onSubSubcategoryChange={handleSubSubcategoryChange}
      />

      {/* Filtros específicos para termostato */}
      <ThermostatFilters
      ...
      <PartsToolbar
        category={{
          ...
          name: 'Termostato',
          icon: 'Gauge',
          description: 'Termostatos, sensores de temperatura, válvulas, juntas',
          parts: []
        }}
